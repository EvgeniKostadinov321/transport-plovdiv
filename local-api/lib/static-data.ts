/**
 * Static data: 532 спирки + GPS + 29 линии.
 * Извличаме се от initial HTML на transport.plovdiv.bg/desktop/ при startup.
 */
...
let lastFailureAt = 0
const FAILURE_RETRY_AFTER_MS = 5_000
...
async function load(): Promise<StaticData> {
  ...
  const stops: Stop[] = rawStops
    ...
    .map((s) => ({ ...s, lines: linesMap.get(s.number) ?? [] }))
...
export async function getStaticData(): Promise<StaticData> {
  ...
  if (sinceFailure < FAILURE_RETRY_AFTER_MS) {
    throw new Error(
      `static data load failed recently (${Math.floor(sinceFailure / 1000)}s ago)`
    )
  }
  loading = load()
    .then((d) => {
