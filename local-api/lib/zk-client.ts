 ...
 * Стратегия: fresh session per query.
 */
...
async function bootstrap(): Promise<Session> {
  ...
  const listboxUuid = html.match(/'([a-zA-Z0-9_]+)',\{id:'stops_list_list'/)?.[1]
...
async function auPost(session: Session, body: URLSearchParams, zkSid: number): Promise<string> {
  ...
  if (!res.ok) throw new Error(`AU POST failed: ${res.status} (${text.slice(0, 100)})`)
  return text
}

function findNewListitemUuid(filterResponse: string, stopNumber: number): string | null {
  const re =
  ...
  while ((m = re.exec(filterResponse))) {
    if (parseInt(m[2], 10) === stopNumber) return m[1]
  }
  return null
}
...
export async function getStopETA(stopNumber: number): Promise<ETAEntry[]> {
  ...
  if (!newListitemUuid) {
    throw new Error(`Stop #${stopNumber} not found in filter response`)
  }
